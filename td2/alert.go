...
type alarmCache struct {
	...
	SentPoAlarms   map[string]time.Time            `json:"sent_po_alarms"`
...
// alarms is used to prevent double notifications. State is saved on exit and
// restored on start (see saveOnExit in run.go and loadConfig in types.go).
...
// pushoverEndpoint can be overridden in tests.
var pushoverEndpoint = "https://api.pushover.net/1/messages.json"

func notifyPushover(msg *alertMsg) (err error) {
	if !msg.po {
		return nil
	}
	...
	if priority == 2 {
		...
		form.Set("expire", strconv.Itoa(msg.poExpire))
	}
	if msg.poSound != "" {
		form.Set("sound", msg.poSound)
	}
	client := &http.Client{Timeout: 10 * time.Second}
	resp, e := client.PostForm(pushoverEndpoint, form)
	if e != nil {
		l("⚠️ Could not notify pushover!", e)
		return e
	}
	_ = resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("could not notify pushover for %s got %d", msg.chain, resp.StatusCode)
	}
	return nil
}
...
func (c *Config) alert(chainName, message, severity string, resolved bool, id *string) {
	...
	a := &alertMsg{
		...
		po:           c.Pushover.Enabled && c.Chains[chainName].Alerts.Pushover.Enabled,
		severity:     severity,
		resolved:     resolved,
		chain:        chainName,
		message:      message,
		...
		poToken:      c.Chains[chainName].Alerts.Pushover.Token,
		poUser:       c.Chains[chainName].Alerts.Pushover.UserKey,
		poSound:      c.Chains[chainName].Alerts.Pushover.Sound,
		poPriority:   c.Chains[chainName].Alerts.Pushover.Priority,
		poRetry:      c.Chains[chainName].Alerts.Pushover.Retry,
		poExpire:     c.Chains[chainName].Alerts.Pushover.Expire,
	}
	c.alertChan <- a
	c.chainsMux.RUnlock()
	alarms.notifyMux.Lock()
