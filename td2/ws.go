		...
		for {
			...
			case update := <-resultChan:
				...
				if update.Final {
					...
					// Grid slot: actual signing state when bonded; -1 "No Data" sentinel
					// otherwise, so a non-bonded validator doesn't paint every block
					// missed (a real miss while bonded still shows missed/orange).
